...
ReadBuffer::ReadBuffer(std::size_t capacity){
    buffer_.reserve(capacity);
}

void ReadBuffer::append(
    const char* data,
    std::size_t size
){
...
std::size_t ReadBuffer::size() const noexcept{
    return buffer_.size();
}
...
void ReadBuffer::consume(std::size_t size){
    if(size > buffer_.size()){
        throw std::runtime_error("Cannot consume more data than buffer contains");
    }
